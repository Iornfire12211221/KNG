...
Ответь только одним словом: low, medium или high`;
  ...
  // Removed auto-analyze - now only runs on save
  ...
  const handleSubmit = async () => {
    ...
    const submitPost = async () => {
      try {
        ...
        // Анализируем важность с помощью ИИ перед сохранением
          ...
          headerRight: () => (
            <TouchableOpacity onPress={handleSubmit} disabled={isSaving || cooldownSeconds > 0}>
              {isSaving ? (
                ...
                <Text style={[styles.headerSaveText, (cooldownSeconds > 0) && styles.headerSaveTextDisabled]}>
                  {cooldownSeconds > 0 ? `${cooldownSeconds}с` : 'Готово'}
                </Text>
              )}
            </TouchableOpacity>
          ),
        ...
        <View style={styles.form}>
          ...
          <View style={styles.section}>
            <TextInput
              ...
              placeholder={selectedType === 'other' ? "Что происходит?" : "Опишите ситуацию..."}
              placeholderTextColor="#999"
              multiline
              numberOfLines={3}
              maxLength={80}
            />
            <Text style={styles.charCount}>{description.length}/80</Text>
          </View>
          ...
          <View style={styles.section}>
            {isGettingLocation ? (
              <View style={styles.locationLoading}>
                <ActivityIndicator size="small" color="#007AFF" />
                <Text style={styles.locationLoadingText}>Определение местоположения...</Text>
              </View>
            ) : address ? (
              <View style={styles.locationInfo}>
                <MapPin size={16} color="#007AFF" />
                <Text style={styles.locationText}>{address}</Text>
                <TouchableOpacity onPress={() => getCurrentLocation(true)} style={styles.refreshLocation}>
          ...
          <View style={styles.infoNote}>
            <Text style={styles.infoNoteText}>
              {(() => {
                ...
                if (days > 0) {
                  return `Актуально ${days} ${days === 1 ? 'день' : days < 5 ? 'дня' : 'дней'}`;
                } else if (hours > 0) {
                  return `Актуально ${hours} ${hours === 1 ? 'час' : hours < 5 ? 'часа' : 'часов'}`;
                } else {
                  const minutes = Math.floor(lifetime / (60 * 1000));
                  return `Актуально ${minutes} ${minutes === 1 ? 'минуту' : minutes < 5 ? 'минуты' : 'минут'}`;
                }
              })()
            }
            </Text>
