            ...
            >
              ...
              {userLocation && (
                <MarkerComponent
                  ...
                  title="Вы здесь"
                  html={`<div data-role=\"user-marker\" style=\"position:relative;width:var(--size,28px);height:var(--size,28px);pointer-events:none;contain:layout paint;\">\n  <div style=\"position:absolute;left:50%;top:50%;width:100%;height:100%;border-radius:9999px;background:rgba(10,132,255,0.18);border:1px solid rgba(10,132,255,0.28);transform:translate(-50%,-50%) scale(1);transform-origin:center;animation:userPulse 1.8s ease-out infinite;will-change:transform,opacity;\"></div>\n  <div style=\"position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:10px;height:10px;border-radius:9999px;background:#0A84FF;box-shadow:0 0 0 2px #FFFFFF, 0 0 12px rgba(10,132,255,0.45);\"></div>\n  <style>@keyframes userPulse{0%{opacity:.7;transform:translate(-50%,-50%) scale(.9)}70%{opacity:0;transform:translate(-50%,-50%) scale(1.6)}100%{opacity:0;transform:translate(-50%,-50%) scale(1.6)}}</style>\n</div>`}
                  webMarkerOptions={{ sizeWithZoom: true, baseSizePx: 28 }}
                />
              )}
            </MapViewComponent>
          )
