            ...
            >
              ...
              {userLocation && (
                <MarkerComponent
                  ...
                  title="Вы здесь"
                  html={`<div data-role=\"user-marker\" style=\"position:relative;width:var(--size,34px);height:var(--size,34px);\">\n                    <div style=\"position:absolute;left:0;top:0;width:100%;height:100%;border-radius:9999px;background:rgba(0,122,255,0.18);border:2px solid rgba(0,122,255,0.35);animation:pulse 1.6s infinite ease-out;\"></div>\n                    <div style=\"position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:calc(var(--size,34px)*0.36);height:calc(var(--size,34px)*0.36);border-radius:9999px;background:#007AFF;border:2px solid #fff;box-shadow:0 0 0 2px rgba(255,255,255,0.6);\"></div>\n                    <style>@keyframes pulse{0%{transform:scale(0.9);opacity:.75}60%{transform:scale(1.25);opacity:.25}100%{transform:scale(1.55);opacity:0}}\n                    </style>\n                  </div>`}
                  webMarkerOptions={{ sizeWithZoom: true, baseSizePx: 34 }}
                />
              )}
            </MapViewComponent>
          )
