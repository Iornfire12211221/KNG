...
export default function MapScreen() {
  ...
  useEffect(() => {
    if (!mapLoadingRef.current) {
      ...
      // Устанавливаем карту как готовую сразу для избежания белого экрана
      setIsMapReady(true);
      setTimeout(() => {
  ...
  useEffect(() => {
    if (userLocation && mapRef.current && !mapInitializedRef.current && mapInitialized) {
  ...
  const handleMapLongPress = (event: any) => {
    ...
    // Проверяем кулдаун только при попытке сохранить пост, а не при открытии окна
